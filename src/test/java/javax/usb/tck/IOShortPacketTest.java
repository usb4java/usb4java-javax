...
public class IOShortPacketTest
{
...

    {
    	...
    	//Note that this code is based on IOTests.RoundTripTest which handles IRP, IRPList, and Byte array.  Since only IRPs
				...
				sendOUTandIN( testType, transmitList[syncOrAsync],  inPipe, outPipe,
                    ...
                    for ( int k = 0; k<numIrps; k++ )
                    {
                        ...
                        if (INexpectedException[k] != null)
                        {
							...
							{
								Assert.fail("Exception clearing halt on endpoint after short packet exception.");
							}
							}
							else
											{
